---
name: codex-reviewer
description: Use this agent when dispatched by the code-reviewer:start skill to run Codex CLI for an independent local-branch review. Shells out to the codex CLI and captures output. NOT for direct user invocation.
model: sonnet
...
`workspace-write` is used so the agent can write a repro test under `tmp/code-reviews/<branch>/<timestamp>/repro/` if it finds a bug to prove. For Q&A follow-ups, use `read-only` instead.
